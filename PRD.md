...
- **2026-09-06** — **Console footing plinth on the handheld layouts.** The mobile shell's bottom controls (mute · SOCIALS · INVENTORY · ?) were white marks on the flat grey shell body, floating loose on an empty deck with nothing behind them, and the landscape bar padded `env(safe-area-inset-bottom)` *inside* a fixed-height bar — so on an iPhone the row squeezed and the home indicator sat on top of it. Added a **footing plinth** (`components/shell/Footing.tsx`): a `#0D0D0D` band pinned to the base of the deck with a `#FF8AC7` hairline along its top edge, mirroring the black wordmark strip under the LCD so the shell reads as bezel · body · footing. The plinth's height *absorbs* the bottom safe-area inset instead of padding it off the row, so the home indicator lands on black and the buttons keep their full 44px targets. Portrait moved SOCIALS/INVENTORY off the mid-deck onto the plinth (matching landscape) and centres the D-pad/A-B clusters in the space above it. Desktop is unchanged.
- **2026-09-07** — **Reverted the footing plinth.** The black bottom band read as a control bar bolted on, not part of the handheld — the shell is meant to be one flat grey body. Removed `components/shell/Footing.tsx` and the `FOOTING_*` tokens; both handheld layouts go back to the earlier arrangement — utility marks printed straight on the grey body, D-pad + A/B up top in portrait, SOCIALS + INVENTORY centred below them, mute / ? in the bottom corners. The contrast and safe-area concerns the plinth addressed are noted but the flat body is the intended look. The A/B face buttons also move to the brand Bebas Neue letterform.
- **2026-09-06** — **Order emails, a four-status lifecycle, and back-office email editing.** Customers previously received nothing after paying — the order landed in Postgres and the buyer was told none of it. Added transactional email via **Resend** (`lib/server/email.ts`, templates in `lib/server/emails/`): an order confirmation sent by the Stripe webhook, a shipping notice, and a thank-you sent when an order is marked delivered. Sends are **awaited but never allowed to fail their trigger** — a dead API key must not cost an order — and both triggers are idempotent: `createPaidOrder` now returns `{ order, created }` so a webhook replay can't email twice, and `setOrderStatus` reports whether the status actually changed. Order status moved from the old two-state model to **paid → making → shipped → delivered** (`0004_order_lifecycle.sql`), with `making` covering the window after Heath places the order with Tapstitch; badge colours encode urgency (red → amber → blue → green). Email copy is no longer in the code: an `email_copy` table (`0005_email_copy.sql`) plus an **Emails** tab in the back office lets Heath edit subject and body for all three templates himself, one at a time behind a tab switcher, with defaults in `lib/server/emails/defaults.ts` as the fallback. Seeded demo orders now carry a visible **sample-data notice** so fabricated revenue can't be mistaken for real. Newsletter sending stays out of scope — it needs list management and unsubscribe handling. Plan: `.claude/plans/okay-so-now-lets-immutable-lerdorf.md`.
- **2026-09-25** — **Pre-launch security hardening.** Patched every known vulnerability in shipped dependencies (`npm audit --omit=dev` is clean): Next.js 15.5.19 → 15.5.26 (critical advisory), `sharp` and `nanoid` bumped, and Next's bundled `postcss` overridden to 8.5.28 rather than jumping to Next 16. Remaining audit findings are dev-only test tooling (`vitest`/`vite`), which never ships. Added site-wide security headers in `next.config.mjs`: a Content Security Policy that loads everything from `'self'` (fonts are self-hosted, Stripe is a redirect), `frame-ancestors 'none'` + `X-Frame-Options: DENY` so the back office can't be clickjacked, HSTS, `nosniff`, a strict referrer policy, and a Permissions-Policy that disables camera/mic/location. The back office's email preview iframe is now sandboxed. Card data never touches SCR!PTS (Stripe hosted Checkout); customer data is protected by Supabase RLS plus the server-only service role. Account-level hardening (2FA everywhere, `ADMIN_EMAIL` set in production, Supabase sign-ups disabled) is the remaining launch checklist.
