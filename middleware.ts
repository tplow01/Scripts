 ...
 *
 * With Supabase unconfigured there is no account to sign in to. In development
 * the gate stands aside and the admin keeps working on seed data; in production
 * it fails closed with a 503.
 */
export function middleware(req: NextRequest) {
  ...
  if (!configured) {
    // Local development: stand aside so the seed-data back office works.
    if (process.env.NODE_ENV !== 'production') return NextResponse.next()
    // Production: a missing env var must not leave the back office open.
    return new NextResponse('The back office is unavailable.', { status: 503 })
  }
