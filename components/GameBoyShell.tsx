...
import ShellFooting, { footingHeight } from './shell/Footing'
...
export default function GameBoyShell({
  ...
  // ── LANDSCAPE / TABLET: LCD centre; D-pad + A/B in flanks; utilities on the
  // footing plinth (mute | SOCIALS+INVENTORY | ?) so they never overlap the game.
  if (layout === 'landscape') {
    const utilBarH = Math.round(Math.max(48, Math.min(0.14 * vh, 64)))
    // The plinth grows by the bottom safe area; everything above stops on top of
    // it, so the home indicator never lands on the utility row.
    const deckBottom = footingHeight(utilBarH)
    ...
    return (
      <div style={{ ...rootStyle, position: 'relative', height: '100dvh', overflow: 'hidden' }}>
        <div style={{ position: 'absolute', left: flankW, right: flankW, top: 0, bottom: deckBottom }}>
        ...
        <div style={{
          position: 'absolute', left: 0, width: flankW, top: 0, bottom: deckBottom,
        ...
        <div style={{
          position: 'absolute', right: 0, width: flankW, top: 0, bottom: deckBottom,
          display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center',
        ...
        {/* Footing plinth: mute left · SOCIALS + INVENTORY centre · ? right */}
        <ShellFooting
          rowHeight={utilBarH}
          muted={muted}
          onUtility={onUtility}
          pillWidth={dmgPill}
          gap={Math.min(20, 0.04 * vw)}
          iconSize={18}
          edgePad={8}
        />
      </div>
  ...
  // ── PORTRAIT: full-bleed LCD top 50%, flat grey deck below, black footing
  // plinth along the base carrying the utility row.
  ...
  // Row height tracks the deck so short phones don't hand the plinth half the
  // shell; the safe-area inset is added on top of it inside ShellFooting.
  const footingRowH = Math.round(Math.max(56, Math.min(0.09 * vh, 76)))
  return (
    <div className="w-screen flex flex-col" style={{ ...rootStyle, height: '100dvh' }}>
      ...
      <div style={{ flex: 1, position: 'relative', minHeight: 0 }}>
        {/* Control clusters, centred in the deck above the footing */}
        <div style={{ position: 'absolute', left: 0, right: 0, top: 0, bottom: footingHeight(footingRowH) }}>
        ...
        {/* Footing plinth: mute left · SOCIALS + INVENTORY centre · ? right */}
        <ShellFooting
          rowHeight={footingRowH}
          muted={muted}
          onUtility={onUtility}
          pillWidth={dmgPillWidth}
          gap={20}
          iconSize={20}
          edgePad={14}
        />
