 ...
 * top-left tile. These are art only — collision, the cashier hole, and the
 * "talk to the cashier" behaviour all come from the single `checkout`
 * interaction in `mainRoom.ts` (its 2×5 footprint minus 4 holes is exactly
 * these six tiles). They are deliberately NOT `solid`: a solid decoration also
 * gets a contact shadow, and a row of shadows down the counter front (one of
 * them landing on the entrance row) read as stray smudges.
 */
